    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            if self.share_across_batch:
                self.mask = (self.rng.uniform(size=(1, inputs.shape[1], inputs.shape[2], inputs.shape[3])) < self.incl_prob).astype(float)
                return inputs * self.mask
            else:
                self.mask = (self.rng.uniform(size=inputs.shape) < self.incl_prob).astype(float)
                return inputs * self.mask
        else:
